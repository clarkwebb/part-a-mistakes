...
Date: 2025-05-19
This file contains the function to solve Laplace's equation for fluids
"""
...
def solve_laplace(init_psi, fixed_psi):
    """
        ...
        for j in range(psi_size):
            for i in range(psi_size):
                ...
                if not fixed_psi[i][j]:
                    res[i][j] = (
                        (psi[i][j + 1] if j + 1 != psi_size else 0)
                        + (psi[i][j - 1] if j != 0 else 0)
                        + psi[i + 1][j]
                        + psi[i - 1][j]
                        - 4 * psi[i][j]
                    )
